...
describe('test order', () => {
  ...
  test('test get user order rejected', () => {
    const newState = orderSliceReducer(
      initialState,
      getUserOrders.rejected(new Error('error'), 'test_id')
    );
  ...
  test('test get user order fulfilled', () => {
    const newState = orderSliceReducer(
      initialState,
      getUserOrders.fulfilled([actualOrder], 'test_id')
    );
  ...
  test('test get new user order pending', () => {
    const newState = orderSliceReducer(
      initialStateNewUserPending,
      newUserOrder.pending('loading', [])
    );
  ...
  test('test get new user order rejected', () => {
    const newState = orderSliceReducer(
      initialState,
      newUserOrder.rejected(new Error('error'), 'test_id', [])
    );
  ...
  test('test get new user order fulfilled', () => {
    const newState = orderSliceReducer(
      ...
      newUserOrder.fulfilled(actualStateNewOrders, 'test_id', [])
    );
