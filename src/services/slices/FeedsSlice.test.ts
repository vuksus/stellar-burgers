...
describe('test feeds', () => {
  ...
  test('test all feeds pending', () => {
    const newState = feedsSliceReducer(
      initialStateFeedsPending,
      getAllFeeds.pending('loading')
    );
  ...
  test('test all feeds rejected', () => {
    const newState = feedsSliceReducer(
      initialState,
      getAllFeeds.rejected(new Error('error'), 'tests_id')
    );
  ...
  test('test order by number pending', () => {
    const newState = feedsSliceReducer(
      initialStateFeedsPending,
      getOrderByNumber.pending('string', 1)
    );
  ...
  test('test order by number rejected', () => {
    const newState = feedsSliceReducer(
      initialState,
      getOrderByNumber.rejected(new Error('error'), 'tests_id', 1)
    );
  ...
  test('test order by number fulfilled', () => {
    const newState = feedsSliceReducer(
      initialState,
      getOrderByNumber.fulfilled(actualStateOrder, 'test_id', 1)
    );
